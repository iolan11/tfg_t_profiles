...
def calcSumT1(l, z, alpha, x, kappa, t2):
    ...
    while i < n:
        ...
        arg1 = ((2*i + 1)*l - z + alpha*x)/((4*kappa*t2)**(1/2))      
        sum_add = ma.erfc(arg1)
        
        arg2 = ((2*i + 1)*l + z + alpha*x)/((4*kappa*t2)**(1/2))      
...
for i in range(0, len(z_T1)):
    ...
    sum_aux = calcSumT1(l, z_T1[i], alpha, x, kappa, t2)
    T1.append(ta * np.exp(-(alpha * x)/l) * sum_aux)
...
for j in range(0, len(z_T2)):
    ...
    arg_aux = (alpha * x + z_T2[j] - l)/((4*kappa*t2)**(1/2))
    T2.append(ta * np.exp(-(alpha * x)/l) * ma.erfc(arg_aux))
...
for k in range(0, len(z_T3)):
    ...
    T3.append(ta * np.exp(-(alpha * x)/l) * ma.erfc(arg_aux))

T = T1 + T2 + T3
z = z_T1.tolist() + z_T2.tolist() + z_T3.tolist()

p = plt.plot(T, z, '-', color='blue')
